...
def listar_visitas_ativas(
    ...
    visitas_ativas = (
        ...
        .filter(
            models.Visita.data_saida.is_(None),
            models.Visita.visitante_id.isnot(None),
        )
        .all()
    )
...
def get_historico(
    ...
    """
    Rota para listar todas as visitas e filtrá-las por datas.
    """
    ...
    for v in visitas:
        visitante = (
            ...
            .filter(models.Visitante.id == v.visitante_id)
            .first()
        )
        resultado.append(
            {
...
def historico_visitante(
    ...
    return (
        db.query(models.Visita)
        .filter(models.Visita.visitante_id == visitante_id)
        .all()
    )
...
@router.put(
    "/visitantes/{visitante_id}/alterar-motivo",
    response_model=schemas.VisitanteOut,
)
def alterar_motivo(
    visitante_id: int,
    motivo: schemas.MotivoRequest,
    db: Session = Depends(get_db),
    user=Depends(get_current_user),
):
    visitante = (
